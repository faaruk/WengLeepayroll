...
namespace WengLeePayroll
{
    public partial class frmDepartment : Form
    {
        ...
        private void buttonLastest1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (PayId == "")
                {
                    ...
                    objSD.SaveDepartments(strCDepartment);
                    MessageBox.Show("Save Successful.");
                }
                else{
                    SaveData objSD = new SaveData();
                    objSD.UpdateDepartments(PayId, strCDepartment);
        ...
        private void buttonLastest3_Click(object sender, EventArgs e)
        {
            ...
            {
                ReadData objRD = new ReadData();
                DataTable dt = objRD.GetDepartmentCount(PayId);
                string cnt = dt.Rows[0][0].ToString();
                if (cnt != "0")
                {
                    MessageBox.Show("This Department is in use, Please check in employee list. Can't be deleted.");
                    return;
                }
                var confirmResult = MessageBox.Show("Are you sure to delete the Department - " + dataGridView1.Rows[i].Cells[1].Value.ToString() + "?",
                                     "Confirm Delete!!",
                                     MessageBoxButtons.YesNo);
                if (confirmResult == DialogResult.Yes)
                {
                    SaveData objSD = new SaveData();
                    objSD.DeleteDepartments(PayId);
                    showdata();
